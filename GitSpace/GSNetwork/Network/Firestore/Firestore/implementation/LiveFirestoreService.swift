...
extension LiveFirestoreService {
  ...
  public func create<T: GSModel>(
    ...
    let docRef: DocumentReference = getDocumentPath(
      superCol: superCollection,
      superDoc: superDocumentID,
      from: collection,
      at: docID
    )
  ...
  public func fetch<T: GSModel>(
    ...
    let colRef: CollectionReference = getCollectionPath(
      superCol: superCollection,
      superDoc: superDocumentID,
      from: collection
    )
  ...
  public func fetch<T: GSModel>(
    ...
    let documents: [QueryDocumentSnapshot] = try await getQueryDocuments(
      from: colRef,
      where: field,
      by: query
    )
...
extension LiveFirestoreService {
  ...
  /// - Author: 원태영
  private func getQueryDocuments(
    from colRef: CollectionReference,
    where field: any FirestoreFieldProtocol,
    by query: FirestoreQueryOperation
  ) async throws -> [QueryDocumentSnapshot] {
    
    let query = makeQuery(from: colRef, where: field, operation: query)
